...
async function main() {
  ...
  // Create sample orders (idempotent)
  let order1 = await prisma.order.findUnique({ where: { orderNumber: 'ORD-001023' } });
  if (!order1) {
    order1 = await prisma.order.create({
      data: {
        ...
        items: {
          create: [
            { productId: products[0].id, quantity: 1, price: 999.99, subtotal: 999.99 },
            { productId: products[2].id, quantity: 1, price: 249.99, subtotal: 249.99 },
          ],
        },
      },
    });
  ...
  let order2 = await prisma.order.findUnique({ where: { orderNumber: 'ORD-001022' } });
  if (!order2) {
    order2 = await prisma.order.create({
      data: {
        ...
        items: {
          create: [
            { productId: products[1].id, quantity: 1, price: 1999.99, subtotal: 1999.99 },
            { productId: products[3].id, quantity: 1, price: 29.99, subtotal: 29.99 },
            { productId: products[4].id, quantity: 1, price: 49.99, subtotal: 49.99 },
          ],
        },
      },
    });
  ...
  // Create activity logs (only if none exist)
  const logCount = await prisma.activityLog.count({ where: { userId: demoUser.id } });
  if (logCount === 0) {
